...
def cnn(X):
    ...
    second_layer_pooled = tf.nn.max_pool(second_layer, ksize = [1, 2, 2, 1], stries = [1,2,2,1], padding = 'VALID', name='pool1')

    third_layer = conv(second_layer_pooled, 112, 3, 64, 128, 1)
    fourth_layer = conv(third_layer, 112, 3, 128,128,1)
    fourth_layer_pooled = tf.nn.max_pool(fourth_layer, ksize = [1, 2, 2, 1], stries = [1,2,2,1], padding = 'VALID', name='pool2')
    ...
    third_third_layer_pooled = tf.nn.max_pool(third_third_layer, ksize = [1, 2, 2, 1], stries = [1,2,2,1], padding = 'VALID', name='pool3')
    ...
    fourth_third_layer_pooled = tf.nn.max_pool(fourth_third_layer, ksize = [1, 2, 2, 1], stries = [1,2,2,1], padding = 'VALID', name='pool4')
    ...
    fifth_third_layer_pooled = tf.nn.max_pool(fifth_third_layer, ksize = [1, 2, 2, 1], stries = [1,2,2,1], padding = 'VALID', name='pool4')

    fc1 = fully_connected(fifth_third_layer_pooled, 7*512, 4096)
    fc2 = fully_connected(fc2, 4096, 4096)
    fc3 = fully_connected(fc3, 4096, 4096)

    output = output_layer(fc3, 4096, 50)
